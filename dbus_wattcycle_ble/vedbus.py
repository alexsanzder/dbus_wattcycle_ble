...
import dbus_wattcycle_ble.ve_utils
    ...
    def _items_changed_handler(self, items):
        ...
        for path, changes in items.items():
            ...
            try:
                ...
                t = str(dbus_wattcycle_ble.ve_utils.unwrap_dbus_value(v))
...
make sure to also subscribe to the NameOwnerChanged signal on bus-level. Or just use dbusmonitor,
...
class VeDbusItemImport(object):
    ...
    def __init__(self,
        ...
        try:
            ...
            self._cachedvalue = dbus_wattcycle_ble.ve_utils.unwrap_dbus_value(v)
    ...
    def _refreshcachedvalue(self):
        self._cachedvalue = dbus_wattcycle_ble.ve_utils.unwrap_dbus_value(
            self._proxy.GetValue())

    @property
    def path(self):
        """ Returns the path as a string, for example '/Dc/V'"""
        return self._path
    ...
    def set_value(self, newvalue):
        ...
        r = self._proxy.SetValue(
            dbus_wattcycle_ble.ve_utils.wrap_dbus_value(newvalue))
    ...
    def _properties_changed_handler(self, changes):
        ...
        if "Value" in changes:
            changes['Value'] = dbus_wattcycle_ble.ve_utils.unwrap_dbus_value(
                changes['Value'])
...
class VeDbusTreeExport(dbus.service.Object):
    ...
    def _get_value_handler(self, path, get_text=False):
        ...
        for p, item in self._service._dbusobjects.items():
            if p.startswith(px):
                v = item.GetText(
                ) if get_text else dbus_wattcycle_ble.ve_utils.wrap_dbus_value(
...
class VeDbusRootExport(VeDbusTreeExport):
    ...
    def GetItems(self):
        return {
            path: {
                'Value':
                dbus_wattcycle_ble.ve_utils.wrap_dbus_value(item.local_get_value()),
                'Text':
                item.GetText()
            }
...
class VeDbusItemExport(dbus.service.Object):
    ...
    @param callback     Function that will be called when someone else changes the value of this VeBusItem
                        over the dbus. First parameter passed to callback will be our path, second the
                        new value. This callback should return True to accept the change, False to reject it.
    """
    ...
    def _local_set_value(self, newvalue):
        ...
        return {
            'Value': dbus_wattcycle_ble.ve_utils.wrap_dbus_value(newvalue),
            'Text': self.GetText()
        }
    ...
    def SetValue(self, newvalue):
        ...
        newvalue = dbus_wattcycle_ble.ve_utils.unwrap_dbus_value(newvalue)
    ...
    def GetValue(self):
        ...
        """
        return dbus_wattcycle_ble.ve_utils.wrap_dbus_value(self._value)
